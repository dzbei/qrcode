...
void ai_draw_init(void)
{
	...
	if(temp1>1)temp1=1;	  
	//使图片处于所给区域的中间            picinfo.S_Width  实际的屏的宽度          picinfo.ImgWidth 实际缩放后的图片宽度
	picinfo.S_XOFF+=(picinfo.S_Width-temp1*picinfo.ImgWidth)/2;   //设置偏移位置
	picinfo.S_YOFF+=(picinfo.S_Height-temp1*picinfo.ImgHeight)/2;
	temp1*=8192;//扩大8192倍	 
	picinfo.Div_Fac=temp1;  //设置缩放系数
