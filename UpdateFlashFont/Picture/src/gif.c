...
u8 gif_decode(const u8 *filename,u16 x,u16 y,u16 width,u16 height)
{
	...
	gfile=(FIL*)mymalloc(0,sizeof(FIL));
	if(gfile==NULL)res=PIC_MEM_ERR;//申请内存失败 
	mygif89a=(gif89a*)mymalloc(0,sizeof(gif89a));
	if(mygif89a==NULL)res=PIC_MEM_ERR;//申请内存失败    
	mygif89a->lzw=(LZW_INFO*)mymalloc(0,sizeof(LZW_INFO));
	...
	if(res==0)//OK
	{
		...
		if(res==0)//打开文件ok
		{
			...
			{	 
				...
				while(dtime--&&gifdecoding)delay_nms(10);//延迟
				if(res==2)
				{
					res=0;
					break;
